...
export default function NewPayablePage() {
        ...
        const fetchData = async () => {
            try {
                ...
                const [supRes, catRes] = await Promise.all([
                    ...
                    axios.get(`${API_URL}/finance/categories?type=EXPENSE`, { headers: { Authorization: `Bearer ${token}` } })
                ]);
                setSuppliers(supRes.data.map((s: any) => ({ value: s.id, label: s.name })));
    ...
    const onSubmit = async (data: any) => {
        ...
        try {
            ...
            await axios.post(`${API_URL}/finance/payables`, formData, {
                headers: {
            ...
            router.push('/dashboard/financial/payables');
        } catch (error) {
            ...
            <Card>
                ...
                <CardContent>
                    <form onSubmit={handleSubmit(onSubmit)} className="space-y-4">
                        ...
                        <div className="space-y-2">
                            <label className="text-sm font-medium leading-none">Anexos (Boletos/Notas)</label>
                            <input
                                type="file"
                                multiple
